...
$('#sub-module-for-1-6').addClass('active')
...
function getMyFeeHistory(){
    $.ajax({
        url: './getStudentDataByGR',
        type: 'POST',
        data: {'Type': 'Student', 'academicSessions': $('#academicSessions').val()},
        success: function(data) {
            if(data.length > 0){
...
$('#proceedPaymentForm').submit(function(e){
    e.preventDefault();
    if(document.getElementById('proceedPaymentForm').checkValidity() !== false){
        $('#proceedPaymentForm').block();
    }
})
...
$('#expiryDate').on('input', function(e) {
    ...
    var newVal = '';
    if(val.length === 2 && this.value.slice(-1) !== '/') {
        newVal += val.substr(0, 2) + '/';
        val = val.substr(2);
    }
    else if (val.length === 3 && this.value.slice(-1) === '/') {
        val = val.substr(0, 2);
    }
    newVal += val;
    this.value = newVal;
...
$('#cardNumber').on('input', function(e) {
    ...
    if (val.length > cardNumberLength) {
        val = val.slice(0, cardNumberLength);
    }
